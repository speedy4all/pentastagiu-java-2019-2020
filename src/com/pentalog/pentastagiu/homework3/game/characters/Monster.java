...
public class Monster extends Character {
    ...
    public Monster(String name, int health, int energy, int x, int y) {
        ...
        if (x < MAP_X && y < MAP_Y && Map.myMap[x][y] == 0) {
            ...
            System.out.println("Bad value for x and y -monster");
        }
    }
        ...
        if (deltaX < DELTA && deltaY < DELTA) {
            if (this.getEnergy() > MIN_ENERGY) {
                if (character.getHealth() > 0) {
                    this.setEnergy(getEnergy() - MONSTER_DAMAGE_ENERGY);
                    if (character.getHealth() > MONSTER_DAMAGE_HEALTH) {
                        character.setHealth(character.getHealth() - MONSTER_DAMAGE_HEALTH);
                    } else {
