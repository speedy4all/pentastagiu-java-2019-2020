...
public class Animal extends Character implements Move {
    private String name;
    private int health;
    private int energy;
    private int x;
    ...
    public Animal(String name, int health, int energy, int x, int y) {
        ...
        if (x < MAP_X && y < MAP_Y && Map.myMap[x][y] == 0) {
            ...
            System.out.println("Bad value for x and y - animal");
        }
    }
        ...
        if (deltaX < DELTA && deltaY < DELTA) {
            if (this.getEnergy() > MIN_ENERGY) {
                if (character.getHealth() > 0) {
                    this.setEnergy(getEnergy() - ANIMAL_DAMAGE_ENERGY);
                    if (character.getHealth() > ANIMAL_DAMAGE_HEALTH) {
                        character.setHealth(character.getHealth() - ANIMAL_DAMAGE_HEALTH);
                    } else {
